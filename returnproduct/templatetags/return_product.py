...
from returnproduct.models import *
...
def calculate_debt(returned):
    total = 0.0
    pwus = ReturnWithUnit.objects.filter(returned=returned)
    for pwu in pwus:
        total += pwu.price
    total = round(total, 2)
    payed = 0.0
    for payment in ReturnPayment.objects.filter(returned=returned):
